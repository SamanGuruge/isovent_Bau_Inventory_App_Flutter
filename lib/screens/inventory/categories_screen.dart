...
class _CategoriesScreenState extends State<CategoriesScreen> {
  ...
  Future<void> _showCategoryDialog({Map<String, dynamic>? existing}) async {
    ...
    final submit = await showDialog<bool>(
      ...
      builder: (dialogContext) => InventoryDialogFrame(
        ...
        subtitle: 'Configure category name, slug, image and status.',
        maxWidth: 600,
        actions: [
          TextButton(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                controller: slug,
                decoration: inventoryInputDecoration(label: 'Category Slug *'),
                validator: (value) => (value == null || value.trim().isEmpty)
  ...
  Future<void> _deleteCategory(Map<String, dynamic> row) async {
    ...
    final used = name.isNotEmpty
        ? await _repo.existsWhere('products', field: 'category', value: name)
        : false;
    if (used) {
      if (!mounted) {
        return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Category is used by products and cannot be deleted.'),
        ),
      );
      return;
    }
    ...
    final approved = await showDialog<bool>(
      ...
      builder: (dialogContext) => InventoryDialogFrame(
        ...
        child: Text(
          'Delete "${row['name'] ?? 'this category'}"?',
          style: const TextStyle(color: AppColors.textPrimary, fontSize: 15),
        ),
      ),
    );
    if (approved == true) {
      await _repo.delete('categories', row['id'].toString());
    }
  }
                          ...
                          child: InventorySurfaceCard(
                            ...
                            child: Row(
                              children: [
                                Expanded(
                                  child: Text(
                                    '${displayValue(row['name'])} • ${displayValue(row['slug'])}',
                                    maxLines: 1,
                                ...
                                InventoryCompactIconButton(
                                  ...
                                  tooltip: 'Edit',
                                  onPressed: () =>
                                      _showCategoryDialog(existing: row),
                                ),
                                InventoryCompactIconButton(
                  ...
                  : SingleChildScrollView(
                      child: DataTable(
                        columns: const [
                          ...
                          DataColumn(label: Text('Category Slug')),
                          ...
                          for (final row in rows)
                            DataRow(
                              cells: [
                                ...
                                DataCell(Text(displayValue(row['slug']))),
